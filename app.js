...
function renderAfterSuggestions(poi){
  ...
  const scored = imageBank.map(img => {
    // Use a defensive approach: if the image does not define any tags then
    // default to an empty array.  This prevents runtime errors when
    // computing the overlap score.
    const tagsArray = Array.isArray(img.tags) ? img.tags : [];
    const overlap = tagsArray.filter(t => wantedTags.has(t)).length;
...
async function loadImageBank(){
  ...
  // Load the image bank from JSON and normalise its structure.  Each item
  // should expose an id, title, url and an array of tag ids.  Older
  // definitions that omit tags or ids are gracefully fixed here.
  ...
  imageBank.forEach((img, idx) => {
    ...
    // Provide a default title fallback if missing.
    if (!img.title && img.label) {
      img.title = img.label;
    }
  });
}
...
function bindUI(){
  ...
  // Update the before photo preview as the user types or pastes a URL.  This
  // makes the experience more visual-first by immediately showing the image
  // without requiring a save.  Clearing the URL field empties the preview.
  el("beforeUrl").addEventListener("input", () => {
    const url = el("beforeUrl").value.trim();
    el("beforePreview").src = url || "";
  });

  // When the solution dropdown changes we update the active POI's solution
  // property and re-open the POI.  This triggers a recomputation of the
  // recommended “after” images based on the newly selected solution tags.
  el("solution").addEventListener("change", () => {
